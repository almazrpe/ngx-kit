...
export class Touches {
    ...
    private toggleEventListeners(
        ...
        for (const listener in listeners) {
            const handler: TouchHandler | MouseHandler | OtherHandler = 
                listeners[listener];
    ...
    private runHandler(eventName: EventType, event: unknown):void {
        if (eventName !== undefined && this.handlers[eventName]) {
    ...
    public on(event: EventType, handler: (event: any) => void): void {
        if (event) {
